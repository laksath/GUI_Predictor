...
def predict_image(input_path):
    op = '\\'.join(input_path.split('\\')[:-2])+'\\loc\\'+input_path.split('\\')[-1].split('.')[0]+'.gui'
    trained_weights_path = 'bin'
    trained_model_name = 'guicode'
    output_path = 'loc'
    ...
    file_name = os.path.basename(input_path)[:os.path.basename(input_path).find(".")]
